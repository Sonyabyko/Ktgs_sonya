for value in collection :
    #Начало блока кода с телом цикла
    #Конец блок с телом цикла
    #Код, кокторый будет выполняться после цикла

for i in range(3):
    # Переменная i будет иметь новое значение внутри цикла на каждом шаге
    print(i + 1, "шаг цикла")
    # будет выполнено 3 раза
 print("Первый цикл закончился!")
a = {1: 'x', 2: 'y', 3: 'z'}
for index, value in a.items():
    print(f'"Позиция: {index} -> Значеник: {value}"')

Список list
for value in [1, 2, 3, 4, 5]:
    print(value)
Кортеж tuple
for value in (1, 2, 3, 4, 5):
    print(value)
Множество set
for value in {1, 2, 3, 4, 5}:
    print(value)

range(start, stop, step)
range(10)
# Генерация последовательности чисел от 0 до 9
range(1, 11)
# Генерация последовательности чисел от 1 до 10
range(1, 11, 2)
# Генерация последовательности чисел от 1 до 10 с шагом 2
range(10, 0, -1)
# Генерация последовательности чисел от 10 до 1 с шагом -1

print(range(10)) # объект типа range
print(list(range(10))) # список целых чисел от 0 до 9
включительно
print(tuple(range(1, 11))) # кортеж целых чисел от 1 до 10
включительно
print(list(range(1, 11, 2))) # список целых нечетных чисел от
1 до 9 включительно
print(tuple(range(10, 0, -1))) # range так же поддерживает
отрицательный шаг

Пример использования _ в цикле:
for _
in range(5): #
Просто выполнить цикл 5 раз
print("Hello World!")

numbers = [10, 20, 30, 40, 50] #объяление списка чисел
total = 0 #TODO Заменить использогвание индексов на перебор значений списка
for i in range(len(numbers)):
    total += numbers[i]
print("Сумма чисел:", total) # Сумма чисел: 150
numbers = [10, 20, 30, 40, 50] #объяление списка чисел
total = 0 #TODO Заменить использогвание индексов на перебор значений списка
for num in numbers:
    total += num
print("Сумма чисел:", total)  # Сумма чисел: 150

for tuple_ in enumerate(["а", "б", "в"]):
    print(tuple_)
В данном примере функция enumerate
используется для перебора элементов списка
["а", "б", "в"].

На каждой итерации цикла, переменная tuple_
будет хранить кортеж, состоящий из индекса и
соответствующего значения.
Результат вывода будет следующим:
(0, 'а')
(1, 'б')
(2, 'в')

for pos, value in enumerate("абв",stsrt = 1) : #start можно указать любым целым числом
    print("Позиция:", pos, "->", "Значение:", value)

Формат записи цикла while
while условия:
    #Конец блока кода с телом цикла
#Код когторый будет выполняться после цикла. Один отступ влево, чтобы закончить объявление цикла

while some_expr: #some_expr - логическое выражение, которое определяет когда нужно остановить цикл
    #Начало блока кода с телом цикла.Один отступ вправо от уровня объявлен6ия цикла
...
...
...
#Конец блока кода с телом цикла.
#Код который будет выполняться полсе цикла. Один отступ влево, чтобы закончить объявление цикла.

count = 0
while count < 5:
    print("Значение count:", count)
    count += 1

 sum_ = 0
 input_number = int\9input("ВВедите число: ")
while input_number !=0:
    sum_ += input_number
    input_number = int(input("Введите число: "))
print("Ответ:, sum_")

Пример бесконечного цикла while:
while True:
# Блок кода, который будет выполняться бесконечно
...

num = 10
while num > 0 :
    print(num)
